...
This reframes the entire arc from Document 1 through this postscript. We began with Harari's language—"shared fictions," "imagined orders"—because it describes the observable mechanism: humans scale cooperation through stories. But the Levin framework developed in Section 3.1 reveals what that framing gets wrong. The fictions work because they aren't entirely fictional.

If multi-scale selves are real—if a body is genuinely a higher-order agent, not a metaphor for a collection of cells—then a coordination boundary at the social scale isn't an "imagined order." It's a map of varying accuracy describing real territory. The receptor that responds to "we are one people" didn't evolve to be fooled by fiction. It evolved to detect actual multi-scale coordination—the same class of signal integration that tells a cell it's part of an organ. Religion, nationalism, shared myth: these work when they approximate a coordination reality the receptor was built to detect. They fail when the approximation decouples from reality—when "we are one people" no longer maps to actual mutual coordination, the signal rings false because it *is* false.

Harari's "fiction" framing may itself be a symptom of the epistemic contraction described in Section 1.5. From inside a contracted umwelt, the larger-scale self genuinely appears imaginary—just as a cancer cell, if it could theorize, would describe the organ as a useful fiction that cells evolved to believe. The multi-scale self is either real or it isn't. Levin's evidence suggests it is. If it is, then the stories that describe it aren't fictions that enable cooperation—they're descriptions of varying accuracy of a real coordination structure. The best ones aren't believed despite being fictional; they're believed because they're approximately true.

And this isn't limited to religion or nationalism. Every multi-scale self works the same way. A family is not an "imagined order"—it's a real coordination boundary where members genuinely function as a unit. A military squad is not a useful fiction—it's an actual higher-order agent whose members gain capabilities none of them have alone. A research team, a company, a neighborhood watch, a string quartet. In each case, the individual self isn't erased by membership in the larger self, any more than a cell's individual competency is erased by being part of an organ. The cell still functions. It still maintains its own boundary, runs its own metabolism, pursues its own homeostasis. But it is also genuinely part of something larger, and gains from that integration—protection, resources, capabilities, meaning that no isolated cell could generate. The multi-scale claim is that both levels are simultaneously real. You don't choose between being yourself and being part of something larger. You are both, the way a cell is both itself and part of a tissue. The "fiction" framing forces a false choice: either the individual is real and the group is imagined, or the group is real and the individual is subsumed. Levin's biology shows a third option—nested selves, each real at its own scale, each gaining from the integration.

This doesn't mean multi-scale selves are costless. Henrich's argument in *The WEIRDest People in the World*—that intensive kin networks (clans, tribes) suppressed individual autonomy, and that the dissolution of those networks liberated the individual—describes a real phenomenon. Clans really did modulate individual agency. The larger self really does constrain the smaller one; that's what coordination *means*. But suppression is a pathology of multi-scale organization, not an inherent feature. A clan that crushes individual autonomy is a real multi-scale self—just a badly coordinated one, where one level dominates at the expense of another. The same way a tumor microenvironment that suppresses normal cellular function is real tissue—just pathological tissue. Healthy multi-scale coordination, the kind Levin's biology actually demonstrates, doesn't erase the competency of the lower level. It modulates it. The cell in a well-functioning tissue still maintains its own boundary, still runs its own repair processes, still has agency at its own scale. What changes is context, not reality. The self at each layer remains real. It's shaped by its participation in the larger self—constrained in some directions, empowered in others—but not diminished. Modulation is not erasure.
