...
#### Discussion: Which Premise the Design Starts From

*This subsection records an unsettled argument rather than a settled mechanism. The pricing rule above is compatible with either starting point; what the starting point determines is which side carries the burden of justification.*

The mechanism above begins with exit and works backward to what departure should cost. There is a prior question it passes over: whether exit or obligation is the premise. The ordering looks academic and is not, because each ordering makes a different thing require defence.

**Starting from exit has a defect that shows up immediately.** If exit is axiomatic, every obligation must be derived from consent—and for the largest obligation any citizen carries, no consent is available. A society transfers on the order of two hundred thousand dollars of public money into each child before adulthood in schooling alone, before counting health, courts, roads, policing, or the accumulated institutional substrate the child will inherit intact. The recipient could not have agreed. Deriving that obligation from consent therefore requires the tacit-consent fiction—that remaining constitutes agreement—which Hume disposed of in 1748: a man does not consent to a ship's master by remaining aboard when he "was carried on board while asleep, and must leap into the ocean and perish, the moment he leaves her." Roughly ninety-six percent of people live in the country of their birth. Almost nobody entered.

The fiction also fails on its clearest cases in the wrong direction. A North Korean born into that state agreed to nothing, and the regime's defining feature is that departure is criminal and punishable against the family. Most of Gaza's population was not alive for the last election. These are the cases where consent language is least available and obligation talk is most urgent, and an exit-first premise has the least to say about them.

**Unchosen collective obligation is the historical norm, and it was infrastructure rather than barbarism.** Before states could identify and pursue individuals, liability ran through groups by design. English frankpledge enrolled every male over twelve in a tithing of about ten households, collectively answerable for producing an offending member and liable for the penalty if they could not. The murdrum fine made an entire hundred pay when a killing went unresolved. Baojia in China bound households into mutually responsible units from the Song reforms onward. Germanic wergild ran liability and compensation alike through the kindred. Medieval commercial law made a community answerable for its members' debts—Avner Greif's **community responsibility system**, which he argues is what made impersonal exchange possible before contract enforcement existed, and which decayed when communities grew too large and heterogeneous to monitor themselves.

The implication runs the same direction as §3.1b of Document 2. **Individual liability presupposes a state capable of finding you.** Collective liability is what a society uses when no such capacity exists. So the shift to "you alone answer for what you alone did"—a genuine moral achievement—was purchased by constructing a heavier apparatus, not by escaping one. The individualist arrangement is again an output of more machinery rather than less.

**The obligation runs forward, which is why declining it does not work.** The childhood transfer is not a bilateral gift from identifiable donors who could be repaid or refused. It is a position in a sequence: each cohort is raised by the one before and owes the one after. There is no counterparty to return the gift to. Burke's formulation is the accurate one—society as a partnership "between those who are living, those who are dead, and those who are to be born."

**And that is a transcendence claim, not a community claim.** The distinction matters, and it is the reason this argument keeps being misfiled. The Postscript's three axes are not interchangeable here: **community** binds a person to contemporaries they can see and be seen by, and generates obligations that are in principle reciprocal within a lifetime. **Transcendence** binds a person to an entity that persists beyond them and that they temporarily instantiate. The intergenerational chain is available only on the second axis. Read on the community axis alone it is unintelligible, because the parties to it are dead or unborn and cannot reciprocate.

This produces a specific modern pathology. Where the transcendence sensor is flattened—no entity persisting past the individual lifespan, nothing real above the person—the childhood transfer becomes unreadable, because the only remaining categories are *gift* and *theft*. There is no third box. It is either something freely given, which a recipient may decline, or something extracted, which a recipient is owed back. The obligation reading requires a persisting entity for the obligation to be owed *to*, and that is exactly what the flattened ontology forbids. This is the same structural absence identified in Document 2 §3.1: if only individuals are real, patterns above the individual have no standing, and everything they generate has to be reassigned to individuals or dismissed. Levin's argument that the self is a pattern at a scale rather than a substrate is what restores the third box, and with it the coherence of an obligation to something that outlives its members.

**What follows for the mechanism above, if the premise is reversed.** Two things change, and neither changes the price.

First, the constraint on the person stops being an assertion and becomes a derivation. **The body is unavailable as collateral because holding it lets the state skip the pricing.** A state that can detain a person never has to compute what that person owes; it holds them until they pay whatever figure it names. Body-as-security is precisely the instrument for avoiding a settlement, which is why every regime that has trapped its population has done so under a fiscal or productive justification. The rule is therefore not that the person is inviolable—prisons and extradition are legitimate, and both operate by adjudication for a specific charge—but that **the body may be taken only by adjudication and never against a claim.**

Second, exit changes role. It is not the foundation of the arrangement but its **audit**: a polity that cannot lose members never has to justify what it charges them. Exit is what forces an obligation to be quantified, which is the same function jurisdictional competition performs at the level of policy, subject to the same requirement of a floor.

**The bound the premise requires.** "You hold obligations you did not choose" is also the sentence every conscripting and exit-banning state has used, and the premise does not carry its own limit. The limit is that the obligation must be **finite, computable, and dischargeable**. A claim that cannot be quantified is not a debt; it is an assertion of ownership. That distinction is the entire difference between citizenship and serfdom, and supplying it is what makes unchosen obligation safe to start from.

**Unsettled.** The reversal is not obviously right. Beginning from obligation places the burden on the departing member to prove settlement, which is the posture every extractive state adopts, and the record of states asked to compute their own claims fairly is poor. Beginning from exit places the burden on the polity to justify its charge, which is safer against tyranny and produces the consent fiction above. The mechanism is identical under both; only the default differs when the computation is contested. The position taken here is that obligation is the more honest premise and that the three constraints—finite, computable, dischargeable, with the body excluded as security—are what keep it from becoming the tyrannical one.
