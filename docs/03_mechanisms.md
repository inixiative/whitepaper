...
**Exit rights as selection pressure:** If governance degrades, members leave. In traditional nation-states, exit is expensive. In network states, exit is clicking "leave community." The claim is that this forces continuous legitimacy maintenance.

**That claim is wrong as stated, and it is the load-bearing flaw in exit-based governance.** Hirschman's *Exit, Voice, and Loyalty* (1970) establishes that exit and voice **trade off**: where exit is costless, voice atrophies, because nobody invests in repairing an institution they can leave by clicking. Worse, the members most capable of effective voice—the most mobile, most skilled, most connected—leave first. Costless exit therefore does not discipline a degrading community. It strips out precisely the people who might have arrested the degradation, and accelerates it. A network state whose only accountability mechanism is departure will not be continuously improved; it will be continuously abandoned by whoever still had options, at increasing speed. See *Exit: What It Costs and Who Pays* below.
...
**Unbounded exit hollows the polity.** Ecuador is the clean case. After the 1999 banking collapse a large share of the population emigrated—overwhelmingly those with the skills, capital and connections to be received elsewhere. The country did not simply lose citizens; it lost the specific citizens who constituted its coordination capacity, and each departure worsened the conditions motivating the next. That is the Hirschman spiral running to completion. And headcount understates the loss, because what leaves with an emigrant is not a person and a bank balance. It is a node in a network: the relationships, the institutional knowledge, the reputational standing, the capacity to convene others. A society losing a tenth of its people to selective emigration does not lose a tenth of its capacity. It loses the edges, and by the argument of Document 2 §3.1 the pattern lives in the edges.

**Prohibited exit produces captivity, and captivity invites expropriation.** The reason elites argue for exit rights is not purely self-serving, and dismissing it as such discards the real content. A population that cannot leave can be taxed to any level and stripped of its property with no recourse, because the only check on a state that does not require consent is the credible threat of departure. Every regime that has closed its borders against its own people did so to foreclose exactly that escape. The suspicion is earned.

**Both are true at once, and the standard failure is to pick one.** A government can be genuinely inefficient, badly incentivized to spend well, and extractive *and* be losing revenue it legitimately needs to jurisdictions that supplied none of the value. These are not competing accounts. They are two facts, and a design acknowledging only one produces either a tax haven or a prison.

**The distinction that does the work is between exit of the person and exit of the claim.** A person leaving is a liberty and should be close to free. What cannot be free is departing while continuing to draw on what you departed from.

The Irish structure illustrates the second. When profit generated by customers in one jurisdiction, served by employees in that jurisdiction, protected by its courts and contract enforcement, produced by workers it educated, is booked as income in another jurisdiction that supplied none of it—nobody has exited. The operations stayed. The customers stayed. The dependence on the institutions stayed. Only the tax residence left. That is not the exercise of an exit right. It is the purest available form of remaining inside the within-group contest while leaving the between-group term, and it is available only to entities large enough to afford the structuring.

**The principle follows: exit free for the person, priced against accumulated claim.** The price is not a penalty for leaving and not a wall. It is a settlement—a one-time reckoning of what was accumulated inside the system, assessed when the accumulation leaves. It scales with what the departing party took from the collective rather than with their wish to go: near-zero for someone with little stake, substantial for someone who accumulated position inside the institution and is now leaving with it. Instruments approximating this already exist—expatriation taxes, destination-based taxation, formulary apportionment assigning income where sales, employees and assets actually sit rather than where the intellectual property is registered.

**And the reciprocal requirement, which exit-based designs consistently omit: voice must be cheaper than exit for anyone able to do both.** If departure is one click and reform is a six-month process, the design has guaranteed that its most capable members will never attempt reform. Every reduction in the cost of exit must be matched by investment in the voice channel, or the mechanism selects for abandonment. This is the correction to §5.4 above, and it constrains every mechanism in this document that offers a member the option to leave.

#### Discussion: Where Do Obligations Come From?

*This subsection works through an argument that is not settled. It is written out in full because the conclusion is easy to state and hard to earn, and the steps are where the content is.*

Start with the position almost everyone holds, because it is a good one.

You should be able to leave. Whatever else a state does to you, it should not be able to keep you. Every regime that has sealed its borders against its own people did it for the same reason, and we can all name the regimes. So put that first: nobody gets trapped.

Now make it the foundation and see what happens. If you can always leave, then staying is a choice, and everything the state asks of you is part of a deal you accepted. Taxes, service, the law — you took the terms. If you don't like the terms, go somewhere with better ones. This is clean, it fits how we like to think about ourselves, and it makes every obligation you carry something you agreed to.

Then ask one question that breaks it.

What did your society spend on you before you were old enough to agree to anything?

Schooling alone runs on the order of two hundred thousand dollars per child in a wealthy country by the time they turn eighteen. That is before you count the doctors, the roads, the courts, the police, the clean water, or the institutions the child will inherit already working. Someone paid for all of it. The child agreed to none of it, because a child cannot agree to anything.

So one of two things is true. Either that money creates no obligation at all — you got it, you owe nothing, you may leave the day you turn eighteen with no more debt than a tourist — or the consent story is wrong, because here is an enormous obligation that nobody consented to.

The usual patch is to say the agreement happened later: you stayed, so you agreed. Hume killed this in 1748 and it has never recovered. A man carried onto a ship while asleep does not consent to the captain by failing to jump into the ocean. And this is not a thought experiment. About ninety-six percent of people live in the country they were born in. Almost nobody ever entered anything. They were simply already there.

The patch fails worst exactly where you need it most. A North Korean born in 2005 agreed to nothing; the state was installed by an occupying army before their grandparents were adults, and its signature feature is that leaving is a crime punished against the family you leave behind. Most of Gaza's population was not alive for the last election. These are the cases where we most want to say something about what a person owes and is owed, and "they accepted the deal" is not just weak there. It is false.

So try the other order. Suppose the obligation is simply a fact.

You were born into a society that had already spent on you before you could be asked. You owe it something. Not because you agreed, but because that is the actual situation you are in.

That sounds like a strange thing to build a system on, until you notice it is how nearly every society before ours worked.

In medieval England, every man over twelve belonged to a tithing — about ten households. If one of them stole something and ran, the other nine had to produce him. If they couldn't, they paid. Nobody signed up for their tithing; you were in it by living where you lived. The murdrum fine went further: if a body turned up and no killer was produced, the whole hundred paid. In China, baojia bound households into groups answerable for each other. Germanic law ran wergild through the kindred — your family paid for your killing, and your family collected for your death.

It is tempting to file all of this as primitive, and that is the mistake. Avner Greif's work on medieval trade shows what it was actually for. He calls it the community responsibility system: a merchant from Genoa could be trusted in London because Genoa was answerable for him. That is what made trade between strangers possible before any court could enforce a contract across a border. Collective liability was not the absence of an institution. It *was* the institution. It broke down later, when communities got too big and too mixed to keep watch on their own members.

Now notice what replaced it, because this is the part that keeps repeating.

"You alone are responsible for what you alone did" sounds like less coercion. It is a genuine moral advance — you should not hang for your cousin. But you can only hold *you* responsible if somebody can find *you*. That takes registries, birth records, identity documents, a police force, courts that can locate a specific person and compel him. Collective liability is what a society uses when it *can't* do that. So the individual version did not free us from machinery. It required much more of it. That is the same shape as the rights argument in §3.1b of Document 2, and it turns up every time: the individualist arrangement is the expensive one.

Fine. Say the obligation is real. Then here is the next question, and it is the hard one.

**Who is it owed to?**

Not to the people who paid. Most of them are dead. The rest are old and did not keep a ledger with your name in it. You cannot repay them and they are not asking. If this were a loan there would be a lender, and there isn't one.

You pay it forward instead. You were raised by the cohort before you and you raise the cohort after. It is a chain, and every generation is standing in the middle of it. Burke had the accurate version in 1790: society as a partnership "between those who are living, those who are dead, and those who are to be born."

Which is why "I never asked for it" does not get you out. You can decline a gift, because a gift has a giver standing in front of you holding it. You cannot decline a position in a sequence you are already standing in.

Here is where the argument keeps getting misread, and the fix is a distinction we already use elsewhere in this document.

The Postscript lays out three sensors: autonomy, community, transcendence. **Community** is about the people around you — the ones you can see, who can see you, where the favor you do today comes back within a lifetime. **Transcendence** is about being part of something bigger than yourself that keeps going after you stop.

The chain only makes sense on the second one.

Try to read it as a community obligation and it falls apart immediately, because the other parties are dead or not born yet. They cannot see you. They cannot reciprocate. There is no relationship there in the ordinary sense. That is why people who hear this argument tend to file it as communitarian, find that it doesn't work as a community claim, and dismiss it. They are correct that it fails as a community claim. It was never one.

And now the interesting consequence.

Suppose you have flattened the transcendence sensor. Suppose nothing above the individual is really real — no entity that persists past a lifespan, only people and their choices. What can the school money be?

There are only two options left. It is a **gift**, which means it was freely given and you may decline it and walk away clean. Or it is **theft**, which means it was taken from someone under compulsion and somebody is owed it back.

That's the whole menu. And the true answer is neither. The true answer is that it is an obligation to something that outlives everybody who is currently in it — and that thing has nowhere to exist in an ontology where only individuals are real.

There is no third box. This is exactly the structure described in §3.1 of Document 2: if patterns above the individual have no standing, then everything those patterns generate must be forced into an individual-level category or thrown away. Levin's argument that a self is a pattern at a scale rather than a particular lump of matter is what puts the third box back — and with it, the possibility of owing something to a thing that will outlast every one of its current members.

So obligations are real and unchosen. Does that mean you can be held?

No. And the argument for that is better than the one people usually reach for.

The usual argument is that the person is sacred and must never be confined. That is not true, and we already know it isn't, because we run prisons and we think we are right to. So sacredness is not the principle.

Here is the actual principle. **If a state can hold your body, it never has to say what you owe.**

Think about how a settlement is supposed to work. You are leaving. There is a number: what you took out of the system, what you put back in, what remains. Somebody has to compute that number, and computing it in public means it can be argued about, appealed, compared to what other people were charged. It is a claim, and claims can be tested.

Now let the state hold you instead. It doesn't need the number. It just keeps you until you pay whatever it names, and the figure can move whenever it likes. Holding the person is not one way of collecting a debt. It is the tool for never having to state the debt in the first place. Which is why every regime that has trapped its population has done it under a productive or fiscal justification — you're needed here, you owe us, you have not settled. The claim is unfalsifiable by design.

So the line is not confinement versus no confinement. It is: **the body may be taken by adjudication, and never against a claim.**

That line does real work, and you can check it against the cases.

A prison sits on the right side. There is a specific charge, a trial, the state carrying the burden, in public, with an appeal. An exit ban sits on the wrong side: a form signed by an official, no charge, often no notice, no appeal, and a fiscal dispute underneath it.

Extradition sits on the right side too, and this is worth noticing because it looks at first like the biggest problem for the principle. States cooperate all the time to fetch people back. But look at what the treaties will *not* do. They require the act to be a crime in both countries. They require a specific charge. They almost universally exclude political offenses. And the common law has long refused to enforce another country's tax judgments — nobody is extradited over a bill. That is the person-versus-claim line, already written into several hundred bilateral treaties, by states that adopted it because it was mutually useful rather than because anyone argued them into it.

Then exit gets a new and smaller job.

It is not the foundation of the arrangement. It is the **audit**. A state that cannot lose you never has to justify what it charges you, because there is no alternative for the charge to be compared against. Exit is what forces the number to exist. That is the same thing jurisdictional competition does at the level of policy, and it needs the same thing: a floor, so that the competition runs on quality rather than on who can free-ride hardest.

Last, the danger, said plainly.

"You have obligations you did not choose" is also the exact sentence used by every state that has conscripted, sealed its borders, or told a population it belongs to the nation. The premise does not come with its own limit, and without one it goes exactly where you would fear.

The limit is three words. The obligation must be **finite, computable, and dischargeable.** There has to be a number. Somebody has to be able to work it out. And once it is paid, it is done.

A claim that cannot be quantified is not a debt. It is a claim of ownership. That is the whole difference between a citizen and a serf, and it is the thing that makes it safe to start from obligation at all.

**Is this settled? No.**

Starting from obligation puts the burden on the person leaving: prove you have settled up. That is the posture of every extractive state that ever existed, and the record of governments asked to compute their own claims fairly is not good.

Starting from exit puts the burden on the state: justify the charge or let them go. That is safer against tyranny, and it forces you back into the consent fiction we started by rejecting.

The mechanism above is the same under either premise. What changes is only who wins when the number is disputed. The position taken here is that obligation is the more honest starting point, and that the three constraints — finite, computable, dischargeable, with the body off the table as security — are what keep it from turning into the thing its critics are right to fear.
...
- **Persistence payoffs.** Continued, diminishing payments are made for each period the policy remains standing. The condition is deliberately binary — did this get repealed? — settled by the policy's scheduled reconfirmations and its standing challengeability (6.5.8), never by a measured index. When a policy is repealed or fails reconfirmation, persistence payoffs cease for future periods (not clawed back for past).

**Why no KPI multipliers.** Earlier drafts attached bonus multipliers to "objective KPIs." This is deliberately removed: any named metric that binds compensation is a Goodhart target, and a leader who nominates their own KPI will nominate a hittable proxy. The only signal ungameable at the measurement layer is the maximally broad did-this-work poll, because it is the terminal value rather than a proxy for it. Narrow metrics still exist in the system — as staked claims (6.7.3) that the proposer posts voluntarily. Staked claims resolve against reputation and the stake itself, never against the compensation stream. The same doctrine governs the persistence trigger itself: an earlier draft conditioned persistence pay on the broad signal exceeding its pre-policy baseline, and that condition is deliberately removed too. A measured baseline reintroduces exactly what this section exists to eliminate — a formula standing between the people and the payout — and a formula is attackable where a vote is not: baselines can be timed (propose in a trough, harvest mean reversion), drift with expectations over decades (a society can govern better and, by habituation, feel the same), and break entirely when the sensor questions themselves rotate. The review vote *is* the maximally broad did-this-work poll, in decision form; survival through reconfirmation is the condition, and there is nothing between the people and the payout to litigate or capture. The entrenchment loophole — a policy persisting only because it built a dependent constituency — is closed by the review machinery rather than by measurement: reconfirmation is scheduled, challenge is cheap and open to anyone, bundles decompose under severability, and a dependent constituency must therefore win an explicit, contested public vote at every review. A repeal driven by habituation remains possible — but it is a deliberate collective act argued against by a paid opponent, a categorically better failure mode than a formula silently zeroing out.
...
**Preserved exit rights:** Can leave your cell and join another if you're dissatisfied. Cells compete for members through quality representation. This mechanism assumes exit is a *supplement* to voice, not a substitute for it—instant recall above is the voice channel, and it must remain cheaper than departure for anyone able to do both, or the cell loses its most capable members before it loses its worst representatives.
...
Two pieces of physiological evidence support this. First, the base rate of sociopathy: only 1–4% of the population lacks the internal coordination receptors (guilt, shame, empathy-as-felt-signal). That's the signature of hardware under strong selection pressure—nearly universal distribution with only statistical outliers missing it. If coordination-signal reception were merely cultural, you'd expect far more variance. Instead, the receptor is almost as conserved as vision. Second, mirror neurons: neural hardware that fires both when you act and when you observe another acting, making another's state *your* state at the physiological level. This is self-boundary expansion wired into the motor cortex—not a metaphor for empathy but the literal mechanism by which one organism's signaling enters another's internal loop. Mirror neurons are what you'd design if you wanted individual nervous systems to function as nodes in a larger coordination network. Both findings point the same direction: the hardware for receiving coordination signals from beyond the individual self is old, conserved, and biological, not cultural.
