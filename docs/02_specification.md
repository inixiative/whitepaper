...
**6. Persistent behavior is selected behavior.** In adaptive systems, if an outcome persists, something is selecting for it—regardless of stated intentions. A system that "keeps failing" at its stated purpose isn't failing; it's succeeding at what it's actually selected for. This is POSIWID restated as selection: the teleology (what a system does) reveals the selection pressure (what's rewarded). Stated purposes are hypotheses; persistent outcomes are data.
