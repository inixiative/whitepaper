...
**Institutions inherit their builders' umwelt.** An institution's sensors are not neutral instruments—they are built by particular people to catch the failure modes those people can imagine. Financial audit, conflict-of-interest disclosure, and procurement review all assume defection is *strategic* (known to its author) and *transactional* (leaves a trace). Both assumptions encode the perceptual bounds of the designers. A defection that is sincerely invisible to the defector, and that moves no resource, satisfies neither assumption and trips no sensor. The institution is not being deceived; it has no organ for the thing. This is the practical consequence of bounded perception at institutional scale: **the failure modes an institution can correct are a subset of those its builders could conceive**, and every uninstrumented channel is an open invitation to whichever competitive strategy runs through it. Re-instrumentation is therefore not a maintenance task but a survival requirement whenever the composition, technology, or incentive structure of participants shifts. See §2.12A in Document 1 for the diagnosis this generalizes.
