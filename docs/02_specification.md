...
A "self," then, is not an object but a *classification*—a way of drawing a boundary around a pattern and saying "this persists, this is a unit." And that classification is **scale-independent**. A cell is a self-propagating pattern. A tissue is a higher-order pattern composed of cell-patterns. An organism is a pattern of tissue-patterns. A colony, an institution, a civilization—patterns all the way up. At every scale, the same question applies: is the signaling substrate strong enough to hold the pattern together? When it is, the pattern persists and we call it a "self." When it isn't, the pattern dissolves into its components, each of which reverts to maintaining only its own smaller pattern.
