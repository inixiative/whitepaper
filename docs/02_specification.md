...
This is the Postscript's first principle applied recursively. "Reality selects for stability" — physics and chemistry produce persistent structures, configurations that endure. Before life, before replication, the universe is already filtering: what persists? What is stable under perturbation? Every subsequent layer in the progression faces the same question. Life answers it with DNA. Intelligence answers it with goal-maintenance. Culture answers it — or fails to answer it — with whatever information infrastructure it manages to build. **Stability is the ur-selection pressure.** Fitness determines which patterns *should* persist. Stability — the stickiness of the information substrate — determines which patterns *actually* persist. These are independent axes: a highly adaptive pattern in a low-fidelity substrate drifts and vanishes; a mediocre pattern in a high-fidelity substrate endures for millennia.
...
**The recurring demotion.** This claim will meet resistance, and it's worth noticing why. Humans have a persistent habit of assuming they are the fundamental unit — and every major intellectual revolution has been a demotion from that assumption. Copernicus: Earth isn't the center. Darwin: humans aren't separate from animals. Freud: you don't control your own mind. Each was resisted for generations by the same psychological mechanism: "but I *feel* like the fundamental unit." Dawkins made the next move — demoting the individual to a vehicle for genes — but then re-committed the same error one level down, treating the gene as the new irreducible center. Levin completes the sequence: there is no fundamental unit. We are not things but patterns — "temporarily persisting, self-reinforcing dynamic patterns" maintained across substrate replacement. It's patterns all the way down, and all the way up. The resistance the reader may feel to what follows is the same resistance that greeted Copernicus.
...
W.D. Hamilton, the evolutionary biologist who formalized kin selection theory in the 1960s, identified the genetic precondition. His rule: an organism will sacrifice its own reproductive success to help relatives when the *relatedness* between them, multiplied by the *benefit* to the recipient, exceeds the *cost* to the helper. In plain language: I'll give up having two of my own offspring if doing so helps my sister raise five, because my genes come out ahead. Eusociality becomes stable when relatedness is high enough that helping the colony *is* helping your genes. Boomsma's monogamy hypothesis (2009) identifies the specific trigger: when a queen mates once and only once, her offspring are equally related to their siblings as they would be to their own potential offspring. The math simplifies—helping and reproducing become genetically equivalent, so any situation where cooperative brood care is even marginally more efficient than going it alone tips the balance toward helping. Hughes et al. (2008) tested this across 267 species spanning **eight independent origins** of eusociality, and every one of those origins traces back to strictly monogamous ancestors. The number carrying the weight is eight rather than 267 — but eight separate origins converging on the same precondition is the stronger result, not the weaker one. The institutional implication is striking—a *mating arrangement* (monogamy) is what opens the door to a major evolutionary transition. Structure precedes cooperation. But genetics only opens the door. What *maintains* eusocial coordination is the signaling substrate—pheromones, tactile communication, shared chemical environment. Destroy the pheromone network and the colony doesn't gradually become less cooperative; it *disintegrates*. Workers stop working. Soldiers stop defending. The superorganism dies the same way a body dies when bioelectric signaling fails: not through destruction of parts, but through loss of the substrate that made parts into a whole.

**This is not an analogy borrowed from biology. It is the same phenomenon at a different scale, and evolutionary biology already has a name for the series.** John Maynard Smith and Eörs Szathmáry's *The Major Transitions in Evolution* (1995) frames the history of life as a sequence of events in which entities capable of independent replication combine into a larger entity that thereafter replicates as one: replicating molecules into chromosomes, prokaryotes into the eukaryotic cell, unicellular organisms into multicellular ones, solitary individuals into eusocial colonies—and, as the final transition in their own list, primate societies into human societies bound by language. The framework is canonical and thirty years old, and it places human social organization in the same series as multicellularity. The claim that groups are real coordinating entities is not a metaphysical addition to biology. It is what the discipline already says.

Their central problem is also ours. Every transition faces the same obstacle: once independent units combine, what stops them defecting? Each transition that succeeded did so by evolving mechanisms that suppress within-group competition—uniparental inheritance of organelles, the single-cell bottleneck each generation, apoptosis, immune surveillance, the strict monogamy that makes eusociality stable. **The transitions were not achieved by the parts becoming trustworthy. They were achieved by architecture that made defection detectable and costly.** That is the design brief of this document, restated in evolutionary terms.
...
#### The Epistemics of Being a Part

One consequence of multi-scale organization deserves stating directly, because it is the source of the strongest modern objection to everything above.

**A part cannot verify the whole.** A cell has no access to the organism's state. It has partial positional information—a voltage gradient, a chemical concentration—and it acts on that. Its perspective is valid and irreducibly incomplete, in the way a frame of reference in relativity is valid and incomplete. This is the light-cone constraint applied to membership: the same bounded perspective that makes local competency possible also makes global verification impossible from the inside.

Modernity took that incompleteness as an *invalidation of the larger entity* rather than as an acknowledgment of the part's own limits. If I cannot verify it, it is not real; if it is not real, I owe it nothing; if I owe it nothing, I will not participate. The inference fails at the first step. Unverifiability is a fact about the observer's position, not about the observed entity.

And the demand for certainty-before-participation is not the cautious position. It carries a guaranteed cost—the entire cooperative surplus—and gets adopted anyway because the cost is diffuse and unattributable while the risk is vivid and has a name. Diego Gambetta's formulation ("Can We Trust Trust?", 1988) makes the trap explicit: trust is only a meaningful concept under uncertainty, since under certainty it would be unnecessary; so requiring proof before extending trust guarantees you never extend it, because the evidence that would justify trusting can only be generated by trusting first. Niklas Luhmann's account is the complement—trust is the device by which an agent reduces unmanageable social complexity to something it can act on at all. An agent that refuses to trust has not become more rational. It has become unable to act.

The formal statement is the stag hunt. Mutual cooperation and mutual defection are *both* stable equilibria, and which one a population occupies is determined by beliefs about the other players rather than by the payoffs. Generalized skepticism is therefore self-fulfilling: it manufactures the world that vindicates it.

**None of which is an argument for credulity.** The correct response to the risk is not to lower the burden of proof but to build the mechanisms that lower the *cost* of meeting it—monitoring, graduated sanctions, reputation that persists, defection that is detected and answered. That is what evolution did at every transition, and what the rest of this specification attempts at civilizational scale. Toshio Yamagishi's finding sharpens it: high-trust individuals are not more gullible but measurably *better* at detecting deception. Trust and discernment are complements, not opposites. What produces gullibility is not trust; it is trust in an environment with no detection.

#### Single-Axis Analysis as Epistemic Shrinkage

If every level of organization is real, and each carries a valid but partial perspective, then an analytic frame recognizing only one level is not merely incomplete. It is the analytic form of the pathology described above.

The dominant frame in the humanities and in much of public discourse reduces social analysis to a single axis: oppressor and oppressed. The reduction is not false. Oppressors and oppressed exist, the axis describes real and consequential relations, and a great deal of history is unintelligible without it. The defect is monism, not content. A framework admitting exactly one axis cannot represent a system in which several are true at once—and multi-scale organization guarantees that several are.

**Two things go invisible under the single axis, and both are load-bearing here.**

*Power structures that are opted into.* The oppressor/oppressed frame codes hierarchy as imposition. But most consequential coordination runs on subordination that was entered voluntarily and is sustained because it pays: the firm, the team, the crew, the discipline, the marriage, the monastery, the unit. This is not an exception to the framework of this document—it *is* the framework. Every major transition consists of parts surrendering autonomy to a larger whole in exchange for access to resources at a scale no part could reach alone. Eusocial workers surrender reproduction. Somatic cells surrender immortality. A frame that reads all hierarchy as oppression **cannot distinguish a transition from an extraction**, because at the level of the part the two are identical: subordination to something larger. It cannot tell apoptosis from murder.

*Every other level of analysis.* Kin, generation, class, region, profession, firm, creed, nation, and species are each real coordinating levels with their own selection pressures, and their claims genuinely conflict. What is optimal for a lineage may be ruinous for its city; what is optimal for a profession may be corrosive to the practice it exists to protect. Conflict between levels is not evidence that one of them is the true level. It is the normal condition of a multi-scale system, and the analytic work is *arbitration among* levels rather than the election of one.

The irony is worth naming, because it insulates the criticism from the obvious rebuttal. Postmodernism's founding move was incredulity toward metanarratives; its applied descendant is a single metanarrative applied without remainder. Foucault's own account of power was explicitly capillary, productive, and diffuse—not a possession of one class wielded against another. The popular form inverts its own sources.

**The consequence is the one this document keeps returning to.** A society that can perceive only one axis of power loses the ability to see coordination that does not fit it—which includes nearly all of the coordination actually running the system, and specifically the kind documented in Document 1. Epistemic shrinkage in a cell produces cancer. In a civilization's analytic apparatus it produces a population fluent in naming one form of power and blind to every other, while the forms it cannot name operate unopposed.

**A corollary about boundaries, which the law has been quietly testing.** If a self is defined by the boundary of what its parts can coordinate on, then an entity's capacity to determine its own membership is not incidental to its existence—it is constitutive of it. A membrane that admits everything is not a membrane, and what it fails to bound is not a cell. The same holds upward: an association that cannot decide who belongs has no boundary, and an entity without a boundary is a crowd rather than a self.

American associational law has spent forty years narrowing exactly this capacity. *Roberts v. United States Jaycees* (1984) held that a state's interest in eradicating discrimination could compel a private association to admit members it had not chosen; *Rotary* (1987) applied the same reasoning; *Christian Legal Society v. Martinez* (2010) upheld a public university's requirement that a student group accept all comers regardless of belief. Campus policy has pushed further than the statutes—Harvard's 2016 sanctions on members of single-gender social organizations, rescinded in 2020—and the direction is consistent.

Each of those decisions answered a genuine abuse. Exclusion is precisely how caste was enforced: the private club where the business actually got done, the restrictive covenant, the segregated professional society.

**Which is the difficulty, stated honestly: exclusion is simultaneously the mechanism by which an association maintains a boundary and the mechanism by which a caste system is maintained. They are the same instrument.** A blanket prohibition removes both. And what survives such a rule is not a landscape of open associations—it is a landscape in which the only durable collectives are those that never needed the right to exclude, because they purchase their coordination through employment contracts instead, and retain through that route near-total discretion over who participates. The rule intended to open the collectives ended by clearing the field for the one kind that could pay its way around it.

**The same corollary at the scale of a polity.** A border is a membership boundary, and everything above applies to it without modification. If a self is the boundary of what its parts can coordinate on, then a polity that cannot determine its own membership is not a polity in the sense used here—it is a territory with a population on it, and the coordination a polity exists to perform has nothing bounding it. This is not an argument about any particular immigration level, which is a policy question whose answers vary with circumstance. It is an argument about what kind of entity is doing the deciding. An entity with no mechanism for deciding its own membership has, by the definition used throughout this section, dissolved as a coordinating self regardless of what its statutes say.

The honest difficulty is identical to the associational one and deserves the same directness: a membership boundary is what constitutes a collective self, and it is also the instrument by which every historical exclusion was carried out. The closed nation is a real thing and so is the closed caste. The same verb, two operations.

Two failure modes bracket the question, and treating either as the only one is how this argument usually goes wrong. A boundary held too tightly ossifies—foreclosing the inflow of talent and capability that every durable polity in history has absorbed, and converting membership into inherited rank, which is precisely the pathology Document 1 documents. A boundary that cannot be held at all removes the entity's capacity to coordinate on anything, including on its obligations to its existing members, and by the argument of the preceding section leaves the field to the collectives that never required a membership boundary because they purchase their coordination instead. Neither failure is the safe default. The design question is calibration, not direction.

The design requirement follows, and it is neither "more freedom of association" nor "less." It is a rule capable of distinguishing an association selecting for *shared purpose*—boundary maintenance, without which no collective self exists—from one selecting for *inherited caste*, which is the substrate consumption this document is about. Those are different operations that happen to share a verb. Any mechanism proposed in this specification that permits group formation must be able to tell them apart, and that requirement is carried forward into the identity and association designs in Document 3.

#### Both Levels at Once: The Standard Misreading

A recurring error in reading everything above is to treat the levels as alternatives—as though establishing that groups are real selective units implied that competition *within* them had been suspended, or that documenting fierce internal competition refuted the group level. Neither follows. Both run simultaneously, always, and the formal apparatus for saying so has existed since 1970.

The **Price equation** decomposes any selection differential into two additive terms: a *between-group* component and a *within-group* component. These are not rival hypotheses about which level selection "really" operates on. They are two terms in one equation, and both are non-zero in essentially every real population. Wilson and Wilson's compression is the standard one: *selfishness beats altruism within groups; altruistic groups beat selfish groups; everything else is commentary.*

The consequence here is direct. A tribe competing hard against neighboring tribes is *also*, at that same moment, hosting a full-intensity status contest among its own members. Elite factions manoeuvre against each other while the polity they belong to faces a peer rival. The two are not in tension and neither cancels the other. They are the two terms, running concurrently, and what varies across history is only their **ratio**.

That ratio is the governing variable, and it accounts for a pattern otherwise hard to explain. When the between-group term is large—a live external threat, a credible peer competitor, a war requiring the whole population—internal suppression becomes unaffordable, because a faction that cripples its own society's capability to protect its rank loses the group contest and its rank with it. External pressure does not make elites virtuous. It makes suppression expensive. Remove the between-group term and the within-group term runs unopposed, which is the general form of the wealth pump.

This also dissolves the individualism-versus-collectivism dispute rather than settling it. "Is the individual or the group the real unit?" has no answer because the question is malformed. Both terms are always present; a design can shift their relative weight and cannot eliminate either. What a governance system can do is set that ratio deliberately, instead of leaving it to whatever external threats happen to arrive.

**And the ratio is not merely ambient. Functioning groups evolve a governor on it.**

The constraint has a general form: internal competition is permitted, and is capped short of damaging the group's capacity to prevail externally. In animal contests the cap is ritualization—display rather than escalation to lethality—which Maynard Smith and Price formalized in 1973 as the reason intraspecific conflict is so rarely fatal. Killing a rival costs the group a member and risks the winner, so the strategies that persist are those settling rank without destroying capability. Human societies have built the same governor repeatedly in institutional form: treason as the one offence admitting no factional defence, conscription placing the sons of the owning class in the same trench as everyone else, the wartime taxation Scheve and Stasavage identify as the "conscription of wealth," and the general norm that factional conflict suspends at the boundary. None of these are moral achievements. They are the cap, encoded.

**The governor has a load-bearing precondition: the competitors must share the group's fate.** The cap is enforced by the fact that weakening the tribe degrades the winner's own prize. An elite unable to escape the consequences of hollowing out its society faces a hard limit on how far extraction can run, because past some point the extraction destroys the very thing whose command constitutes the rank being competed for.

Remove that precondition and the governor releases. This is the specific damage done by elite exit options—capital mobility, assets and heirs held offshore, citizenship as a purchasable hedge. **An elite with credible exit has been functionally removed from the between-group term while remaining fully inside the within-group term.** It competes for rank against its peers, and the society beneath it is no longer its group but its resource base. This is cancer in the precise sense given earlier: the self-boundary has contracted, and what was the body has become environment.

The signature is diagnostic, and it distinguishes this from ordinary elite competition, which every society has always had. Ordinary factional conflict is fought *over control of* the substrate and therefore retains an interest in the substrate staying productive. Post-governor competition is fought by *consuming* the substrate, because the consumer expects to be elsewhere when the bill arrives. The two resemble each other from inside and produce opposite trajectories. The discriminating test is whether the competing factions are exposed to the outcome. Where they are not, the observed behaviour is not a failure of elite virtue. It is the correct play for a participant who has left the group without announcing it.

#### The Non-Located Elite: What Exit Looks Like at Civilizational Scale

The preceding argument is game-theoretic. Its sociological instantiation in the modern West has been documented independently and repeatedly, and the convergence across otherwise hostile political positions is itself evidence.

Christopher Lasch's *The Revolt of the Elites* (1995) describes a professional and managerial class whose loyalties are international rather than local, which has effectively seceded from national life while retaining power over it. Samuel Huntington reached the same conclusion from the opposite direction in "Dead Souls: The Denationalization of the American Elite" (2004). Saskia Sassen's work on denationalization documents the institutional machinery. David Goodhart's Anywhere/Somewhere distinction is the popular vocabulary for the same split. Four observers with little else in common, describing one phenomenon: an elite that is not located anywhere in particular.

**The mechanism is asymmetric mobility, and it was a choice rather than a natural development.** Dani Rodrik's argument (*Has Globalization Gone Too Far?*, 1997) is the precise one: liberalization opened capital and goods to free movement while leaving labour mobility restricted, and that asymmetry is a decision about *whose* mobility counts. Its effect is to raise the elasticity of demand for labour—capital can credibly threaten to leave, labour cannot—transferring bargaining power without any party needing to intend it. Branko Milanović's finding that citizenship is now the dominant term in global inequality measures the same fact from the other side: capital moves, people do not, so where you were born determines most of your income.

**Hirschman supplies the theory, and it is not the one usually attributed to him.** *Exit, Voice, and Loyalty* (1970) is commonly reduced to "exit rights discipline institutions"—if you dislike it, leave. Hirschman's actual and more troubling finding is that exit and voice **trade off**: where exit is cheap, voice atrophies, because nobody invests in repairing an institution they can simply depart. Worse, the members most capable of effective voice are typically the first to leave. Cheap exit therefore does not merely fail to discipline a declining institution—it strips out precisely the people who might have arrested the decline, and accelerates it. Applied to an elite this is exact: an elite with cheap exit has no reason to exercise voice on behalf of the polity, because departure is always cheaper than repair, and it is the most mobile who go first.

**This bears on a tension inside this specification that should be named rather than left implicit.** The exit-based reading treats exit rights as an unambiguous good—multiple communities, leave the one you dislike—and for an individual facing a captured or abusive local institution they are. Exit is modernity's central liberating achievement: the ability to leave the village, the family business, the creed, the marriage. That is not a cost to be regretted; it is what made individual autonomy real. But the same instrument, distributed asymmetrically, removes the governor described above. Exit is what protects a person from a predatory collective *and* what permits a predatory elite to stop being a member. The design question is therefore not whether exit rights are good. It is **who holds them, at what cost, and whether that cost scales with the holder's power over those who lack them.**
...
### 3.1c Costly Display: Why Rational-Choice Models Misread Culture

Start with something ordinary.

A capable person finishes school at eighteen. They then spend four years on a degree, often two more on a master's, sometimes three or four beyond that. They emerge in their late twenties carrying debt, into a field where the credential is required for entry and confers no advantage once inside it, because everyone else has one too. Along the way they have spent the years in which fertility is highest and family formation is easiest. Many of them will not have children as a result, and most of them know this while it is happening.

Ask what is being maximised there. It is not lifetime earnings — the marginal degree frequently fails to pay for itself. It is not security. It is not health, or time, or offspring. Whatever the process is optimising for, it is not any of the things economics assumes people want.

The standard reply is that people are irrational: subject to biases, poor at discounting, bad at long horizons. This lets the framework stand and adds patches. But it is a strange thing to say about a behaviour that is near-universal, highly systematic, and pointed consistently in one direction. Random error looks like noise. This looks like a strategy.

**Veblen named it before economics had a way to model it.** *The Theory of the Leisure Class* (1899) described conspicuous consumption: display whose function is precisely that it is wasteful, because only someone with surplus can afford the waste. The uselessness is not a side effect. It is the message. Veblen was describing the same structure biology would arrive at seventy-six years later, and he was describing it in humans first.

**The biology, and what in it is actually settled.** The peacock's tail is the canonical case: heavy, conspicuous to predators, metabolically expensive, and reliably selected for by peahens. Zahavi's handicap principle proposed that the cost is what makes the signal honest — only a genuinely fit male can afford the burden. Fisher's runaway offers a different route: once a preference exists, it amplifies itself. Neither fully answers the question Bret Weinstein has pressed, which is why a female would choose, for her own sons, a trait that visibly reduces their fitness.

And the standard explanation is in worse shape than that. Penn and Szamadó (2020) argue that the handicap principle is "an erroneous hypothesis that became a scientific principle," and that newer models show signals need not be costly to be honest — what deters cheating is the *potential* cost of being caught, not the realised cost borne by honest signallers.

This needs stating plainly rather than deferring to a caveat at the end, because it determines what this section is allowed to lean on. **The mechanism is contested. The pattern is not.** Organisms across an enormous range of species observably select for conspicuously expensive display; the offspring bear the cost; the selection persists anyway. Nothing below requires Zahavi's explanation to be correct. It requires only that costly display is a real and stable strategy, which no one disputes. Where this document uses the phenomenon, it uses the observation and not the theory of why.

**The move that resolves it is not to abandon rationality but to change the objective.** The critique that humans are irrational keeps the utility function and blames the agent. The alternative keeps the agent and questions the function.

Suppose people are rational optimisers, and the thing being optimised is **relative reproductive positioning** rather than material welfare. This is not an exotic assumption. It is the only objective natural selection is capable of writing, for the reason given in §3.1: fitness is a ratio, and a trait spreads by out-replicating its alternatives rather than by raising anyone's absolute output.

Run the catalogue of "behavioural anomalies" through that objective and they stop being anomalies:

- Credential stacking that destroys your twenties is irrational under lifetime earnings. It is rational under *signalling quality relative to your cohort*, where the burden borne is itself the evidence.
- Luxury beliefs (Henderson) that impose real costs on the holder's community and children are inexplicable under material utility. They are efficient displays under intrasexual competition, because a position you can afford to hold while others cannot is exactly what a costly signal is.
- Consumption calibrated to signal rather than to build resilience, parenting norms that escalate expense per child (§1.2c), careers optimised for prestige over security or family compatibility — the same inversion each time.

Economics files these as inefficiencies, information failures, or exotic preferences. But they are not scattered. They point one way, at population scale, consistently. **The models are not wrong about rationality. They are wrong about what rationality is for.**

**Humans can opt out, and that is where the analogy breaks — but it breaks in an uncomfortable direction.** A peahen cannot choose to prefer flying ability; the preference is hardwired. A person can in principle refuse the game: skip the credential, homeschool, reject the beliefs, optimise for family over standing. So the biological framing should not be read as determinism.

The problem is not that opting out is impossible. It is that **the alternatives have been closed off institutionally.** There is no mixed-age schooling system producing competent adults at thirteen (Gatto's point). There is no credential-free path into most professions, because the credential is a licensing requirement rather than a signal the employer chose to demand. There is no community where rejecting the prevailing beliefs is socially free. Where genuine alternatives survive — trades, some religious communities, the homeschool movement — people do take them, which is the evidence that the capacity to exit is intact. They remain margins because the mainstream landscape has been captured, not because the preference is hardwired.

**And the mechanism eats itself, which is where this joins Turchin.** The systems that sort and suppress — compulsory schooling, credential requirements, professional licensing — simultaneously manufacture the counter-elites they exist to prevent. As the signal inflates, entry costs rise while the number of positions does not. Each cohort pays more for the same or fewer seats. The result is exactly Turchin's formula for instability: a large population with elite self-conception and non-elite prospects.

The people who benefit from the signalling system cannot stop it overproducing rivals, because the overproduction *is* the escalation. You cannot require a master's degree for entry-level work and then be surprised to have produced a generation that is over-credentialed, under-employed, and hostile to the arrangement that took their best years and returned nothing. The suppression generates the instability it was built to prevent.

**Where this framework bottoms out.** Four gaps, stated because a model whose limits are hidden gets applied past them.

1. **Origin.** Costly display explains its own maintenance better than its beginning. Fisherian runaway provides a mechanism but relocates the question to the initial preference. In culture, competitive saturation (Sulikowski) is the plausible trigger — as a hierarchy fills, competition shifts from output to display — but that is a description of the conditions, not a full causal account.

2. **Termination.** In biology the escalation is checked by predation and starvation: the tail eventually kills the bird. In culture the analogous limit may be Tainter's diminishing returns to complexity, with collapse as the ceiling. Where the limit sits, and whether a given society reaches it or pulls back, is not predictable from this model.

3. **Observation.** Per §1.5b, this is water. If costly display pervades a culture, then the evaluative apparatus we would use to detect it was itself shaped inside it. The peacock cannot assess whether its tail is adaptive; it simply finds large tails attractive. We may be in that position regarding credentials, beliefs, and status markers.

4. **The Weinstein gap.** Nobody has a satisfying account of why organisms enter these equilibria in the first place, and Penn and Szamadó's critique makes the account of maintenance shakier too. The application here inherits that gap honestly: a real and well-documented pattern, applied to a domain it fits, with the underlying theory unsettled.

**Why this matters for design.** If cultural behaviour runs substantially on display logic rather than market logic, interventions built on rational choice will fail in a specific and predictable way. Better information will not help, because the agents are not misinformed. Marginal incentive adjustment will not help, because the signal *is* the incentive. Appeals to outcomes will not help, because outcomes are not what is being selected for.

What is left is changing the environment rather than the choices inside it — altering what gets rewarded with standing, so that signals tracking real capacity outcompete signals tracking display. That is a requirement on §3.3's incentive design, and it is a hard one, because every institution that currently confers status is a participant in the existing game.
...
**The Invisibility Problem:** The hardest technologies to upgrade are the ones old enough to have become invisible. We recognize smartphones as technology; we don't recognize governance as technology. The Cistercians *invented* systems management—but to us, "management" feels like reality, not invention. Money feels like physics. Property rights feel like nature. Bureaucracy feels like "how organizations work." As Žižek and David Foster Wallace both observed (see the epistemological postscript following Section 1.5), ideology is invisible to those inside it. The fish doesn't see the water.
...
### 3.11 Language Resolution: Vocabulary as Governance Infrastructure

A society's working vocabulary is governance infrastructure, not decoration. The set of distinctions that show up as separate words in ordinary political speech determines what can be debated, regulated, taxed, prohibited, or rewarded. When a single word collapses several distinct phenomena into one referent, the political system loses the ability to treat them differently. Capture happens *through* that collapse, because rent-seekers can hide their activity inside the same word as legitimate activity, and the legitimate activity ends up defending the rent-seeking by association.

**The Orwell point.** This is George Orwell's case, sharpened. *Politics and the English Language* (1946) argued that vague, abstract, euphemistic political language is not a stylistic failing but the active mechanism by which atrocity gets done — "the great enemy of clear language is insincerity"; political prose is constructed precisely "to make lies sound truthful and murder respectable." *Nineteen Eighty-Four* (1949) generalized the point to its endpoint in the Newspeak appendix: a deliberately engineered vocabulary in which heretical thought becomes *literally unspeakable* because the words needed to think it have been removed. The point of Newspeak is not censorship — censorship still presupposes that the forbidden thought can be formed. The point is to make the forbidden thought *unformable*. The collapses documented below are the everyday non-totalitarian version of the same dynamic — produced not by central planning but by drift, professional jargon, euphemism, and the political convenience of incumbents — but the outcome is structurally identical: phenomena that should be governable separately become a single ungovernable referent, and the citizen who tries to draw the distinction finds that ordinary speech no longer contains the words. Orwell saw this in 1946 as the specific signature of totalitarian language; the argument of this section is that the same mechanism now operates in Western political vocabulary through softer means and with the same outcome — and that resisting it is a substrate-level design requirement for any cooperative successor order.

**The resolution principle: one word, many phenomena = capture surface.** A vocabulary collapse is not an aesthetic problem; it is a governance failure mode. Wherever distinct phenomena share a name, two predictable things happen. First, *regulation cannot target the bad without hitting the good*, because there is no linguistic handle to grab the bad alone. Second, *defenders of the bad recruit the good as their political shield* — "you can't regulate this without also damaging that," where "this" and "that" are the same word for two different things. The Greek city-states understood this in reverse: where a distinction mattered for governance, they invented separate words for it (see *parrhesia* and *isegoria* below). Modern Western political vocabulary has run the opposite experiment for fifty years, collapsing distinctions that used to be operative, and the capture costs are now legible.

**Case 1: Maintenance.** It has been observed in post-colonial development that infrastructure inherited from departing colonial administrations decayed catastrophically not from sabotage or hostility but from the absence of a culturally-encoded concept of *scheduled preventive maintenance* — repair as a routine ongoing activity rather than a reactive response to visible failure. Where there is no everyday word for "the work of keeping a working thing working," the work does not get budgeted, scheduled, paid for, or performed. The systems run until they break and then they are gone. This is not a developing-world phenomenon; it is a vocabulary phenomenon. The United States now spends a vanishing fraction of its infrastructure budget on maintenance versus new construction, and the ASCE's most recent Infrastructure Report Card gives existing US stock a C overall (with multiple subcategories at D), a marginal-passing grade across the board. The political class can speak fluently about *building* (ribbon-cuttings, headlines, lobbying coalitions) and has nearly no operative vocabulary for *keeping* (boring, distributed, no political reward). What you can't easily say, you don't easily do.

**Case 2: Money.** The single word *money* in current political-economic discourse refers to at least five structurally distinct phenomena, and the collapse is load-bearing for the present extraction regime:

1. **Productive money:** Wealth generated by creating something a freely-choosing counterparty values enough to pay for — the Randian/Smithian sense, money as a token of consensual productive contribution.
2. **Externalized-cost money:** Wealth generated by producing real costs (pollution, depletion, social harm, systemic risk) that someone else has to pay later. Looks identical to productive money on a balance sheet.
3. **Corruption money:** Wealth captured via political access, regulatory carve-outs, no-bid contracts, insider information. Booked as ordinary revenue.
4. **Monetary-creation money:** Wealth conjured by printing, fractional-reserve expansion, central-bank asset purchases, or sovereign debt issuance — a transfer from holders of existing money to recipients of new money, denominated in the same units as productive earnings.
5. **Asset-inflation money:** Wealth that appears on the balance sheets of those who already own assets when the price of those assets rises faster than the productive economy underneath them. The Gary Stevenson mechanism. Looks identical to productive return.

These are five different substrate phenomena with five different governance implications. By using one word for all five, the political system treats the wealth of a productive small-business owner and the wealth of an asset-inflation rentier as morally and operationally equivalent — and the rentier can credibly invoke the small-business owner as a political shield against any attempt to discipline asset-inflation. The "tax the rich" debate is unresolvable in current vocabulary because *the rich* is itself a collapsed term spanning all five categories. A successor order that wants to discipline categories 2–5 without flattening category 1 must develop, in everyday speech, the vocabulary to name them separately.

**The wealth-tax debate as canonical example.** The current Western argument over wealth taxation is §3.11's thesis caught in real time. The financial-illiteracy critique of wealth taxes — capital flight, valuation gaming, illiquidity, double taxation, capital destruction, perverse incentives against productive investment — is structurally correct *when its implicit referent is Money Category 1 (productive capital)*. France's ISF, Norway's recent capital exodus, the well-documented failure modes of net-wealth taxes on operating businesses: all real, all serious, all addressed at productive money. The Stevenson/Piketty case — without aggressive taxation of asset accumulation, those who already own assets buy *everything* because asset price growth structurally exceeds wage growth indefinitely — is structurally correct *when its implicit referent is Money Category 5 (asset-inflation capital)*. Post-2008 QE/ZIRP transferred trillions to asset holders; institutional investors are buying single-family housing stock at scale; the endpoint is asset-class feudalism. Both sides are right within their own referent. Both sides are wrong about what the other side is referring to. Each camp's accusation that the other is financially illiterate is itself accurate — *for the version of the opposing argument that applies to the wrong category of money*. The debate is therefore unwinnable in current vocabulary, and the unwinnability is itself the mechanism by which nothing happens while the asset-inflation transfer continues. A vocabulary in which "tax on productive capital" and "tax on asset-inflation capital" were separate political objects, with separate procedural tracks, rates, and exemption regimes, would expose what both camps would actually agree about if forced to refer to the same thing: Category 1 should be lightly taxed and Category 5 should be heavily disciplined. That dissolution has not happened because it requires a vocabulary that the rentier coalition has every incentive to prevent and the productive coalition has no organized capacity to demand. The debate stays at the level of *the rich*, and the asset-inflation transfer stays at the level of trillions.

**Case 3: Taxes.** *Taxes* in current vocabulary collapses at least two distinct flows:

1. **Substrate taxation:** Revenue extracted from productive economic activity to fund genuinely public goods (defense against external threats, courts, infrastructure that produces more value than it costs, basic research, the institutional substrate that makes productive activity possible at all).
2. **Coalition-bribery transfer:** Revenue extracted to fund whatever spending the current political coalition needs to pay to remain a coalition — payments that flow because someone organized a constituency to demand them, regardless of whether the resulting spending produces net value. This is the "selectorate" budget (see §2.9), and it grows monotonically because every increment of it builds further constituencies that cannot be cut without losing the coalition that built it.

The political system treats both as "tax-and-spend" with identical procedural status. A simple majority can pass either. The result is the well-documented ratchet: substrate taxation is hard to raise (because the productive class organizes to resist) but easy to defend; coalition-bribery transfer is easy to raise (because each new program creates its own permanent defenders) and impossible to cut. The collapse of the two into one word is what makes the ratchet politically invisible. Until everyday political speech can name "tax-and-spend that funds public goods" separately from "tax-and-spend that funds coalition maintenance," the second will continue to crowd out the first while wearing its clothes.

**The de-concentration trap.** There is a structurally sound case for aggressive taxation of concentrated private wealth: concentrated private wealth produces concentrated power, and concentrated power is the harm (this is the Stevenson/Piketty argument, and §2.12A's diagnostic agrees with it). But the case dissolves if the resulting revenue flows into the coalition-bribery channel rather than the substrate channel. Taxing private rentiers to fund a growing managerial-bureaucratic apparatus does not de-concentrate power; it *moves* the concentration from the private rentier class to the public rentier class. James Burnham's *Managerial Revolution* (1941) named this in its early stages; the Soviet *nomenklatura* and modern administrative-state lifers are what it looks like in maturity. The substrate harm — power concentration — is invariant under the swap. Wealth taxation as a substrate-aware policy therefore requires *both* sides of the equation: extract from concentrated private accumulation *and* prevent the receiving institutions from converting that revenue into a new concentration. Without the second move, the first is a power *transfer*, not a power *reduction*, and the productive class pays twice — once as taxpayers, once as the eventual subjects of whatever bureaucratic coalition the redistribution funds into permanence. The collapse of these two failure modes into the single debate "tax the rich: yes or no" is itself an instance of the vocabulary problem this section is about.

**Case 4: Regulation.** *Regulation* in current vocabulary collapses at least two structurally opposite phenomena:

1. **Anti-capture regulation:** Rules that constrain concentrated private power — anti-monopoly enforcement, anti-pollution standards, fraud and disclosure law, fiduciary duty, capital requirements that make systemic risk costly to its creators.
2. **Capture-as-regulation:** Rules written by incumbents to raise barriers against entrants, freeze competitive dynamics, mandate the incumbent's preferred technical standards, or convert market competition into regulatory-credit auctions the incumbent always wins. (See §2.11.) Occupational licensing, certificate-of-need laws, complex compliance regimes that scale-favor incumbents, IP regimes optimized for rent extraction rather than innovation incentive.

The two are political opposites — one disciplines capture, the other *is* capture — but they share a word and a regulatory apparatus. "Deregulation" therefore dismantles both indiscriminately, and "more regulation" reinforces both indiscriminately. Neither stance is coherent because the underlying noun spans both phenomena. A successor order needs two words, two political coalitions, and two distinct procedural tracks. Until then, "deregulation" debates will continue to be unwinnable for either side because each side is half right about a referent that doesn't exist as a single thing.

**The positive example: Greek vocabulary resolution.** Classical Athens, whose democracy was load-bearing on free public speech, used *two* words where modern English uses one: *isēgoría* (ἰσηγορία) — the equal right of every citizen to speak in the assembly — and *parrhēsía* (παρρησία) — the freedom to say what one actually thinks, including uncomfortable, offensive, or politically dangerous things. These name different governance properties. *Isēgoría* is a procedural right (access to the floor); *parrhēsía* is a substantive norm (protection from social or legal consequence for what one says once on the floor). The Athenians could discuss them separately because they had separate words for them, and could therefore notice when one was eroding while the other remained intact — or when the political class was performing the form of one to disguise the loss of the other. Modern Western political speech has one word, *free speech*, and consequently cannot distinguish between "you may post on this platform" (a degraded *isēgoría* preserved by terms of service) and "you can say what you actually think without losing your career" (a *parrhēsía* almost entirely lost over the past two decades). The collapse of the two into one word hides the fact that one is gone.

**Design implication.** A successor order must treat vocabulary resolution as part of its institutional substrate, not as a stylistic preference left to writers and rhetoricians. Three implementation directions:

- **Constitutional-level vocabulary:** Foundational documents should name the categorical distinctions that matter for governance (the five money-types, the two tax-flows, the two regulation-modes) and require statute to specify which it is operating on. A bill that does not declare its category is non-justiciable on its face. This is the linguistic analog of a building code requiring electrical and plumbing diagrams to be drawn separately.
- **Procedural-track separation:** Once categories are named, different categories travel different procedural tracks. Coalition-bribery transfer, once distinguished from substrate taxation, can be subject to a higher passage threshold or a sunset rule that substrate taxation is not. Capture-as-regulation, once distinguished from anti-capture regulation, faces a different judicial standard of review.
- **Public-vocabulary maintenance:** Like maintenance of physical infrastructure, vocabulary maintenance is unglamorous, distributed work — but it is the substrate on which everything else runs. Educational institutions, public broadcasters, and civic media in a successor order have an explicit mandate to preserve and propagate the operative distinctions in ordinary speech, the same way they preserve numeracy. A polity that cannot say what it means cannot govern what it has.

The general principle: **the resolution of a society's working vocabulary is an upper bound on the resolution of its governance.** Wherever an important distinction lacks a word, the corresponding policy lever does not exist no matter what institutional architecture is layered on top. Wherever an unimportant similarity is enshrined in a shared word, two phenomena that should be disciplined separately will be defended together. Designing for cooperation at civilizational scale therefore requires designing the vocabulary that scale will be discussed in, not just the institutions that vocabulary will operate.
...
### 4.0 The Catalogue of Governance Failure Modes

Before specifying what a successor system must be, this section catalogues what every prior system *failed at*. The §4 principles that follow are not abstract virtues, nor a wish list of features a good society would have; they are structural prophylactics against named historical failure modes. This converts the design problem from "design an ideal society" (a question that admits infinite arbitrary answers) into "satisfy this checklist of avoidance criteria" (a determinate problem). Every entry below names a governance form, its characteristic failure mode, the canonical historical case, and the underlying mechanism. The §4.x principles map onto these entries: each principle exists to prevent one or more specific failures in this table.

| Governance form | Characteristic failure mode | Canonical case | Underlying mechanism |
|---|---|---|---|
| **Direct democracy (Athens)** | No continuity; vulnerability to demagogues; mass hubris in collective decision | Sicilian Expedition (415 BC); collapse to Thirty Tyrants (404 BC) | Sortition + one-year terms + no consecutive office = zero institutional memory; assembly hackable by rhetoric; no successor training; mass votes amplify hubris |
| **Mixed republic (Rome)** | Elite overproduction → personal armies → civil war | Marius–Sulla–Caesar–Augustus arc (1st c. BC) | Wealth concentration via conquest broke the citizen-farmer base; generals became patrons of armies; Senate could not discipline winners |
| **Imperial succession (Rome)** | No legitimate succession rule → military coup endemic | Third-century crisis (235–284 AD): ~50 emperors in 50 years | Heritability, adoption, and acclamation all failed in turn; legions made emperors; no institutional method for orderly transfer |
| **Feudalism** | Protection bargain hollows; rent persists after protection cost falls | 14th–15th c. peasant revolts (Jacquerie 1358, Wat Tyler 1381, German Peasants' War 1525); Tokugawa samurai stipends | Localized protection genuine when violence cost matched lord-scale capacity; rent extraction continued after gunpowder transferred protection to crown-scale |
| **Absolutism** | Fiscal-military overreach + ideological vulnerability | Bourbon France (1789); late-Romanov Russia (1917) | War debts exceeded substrate capacity; tax reform blocked by privileged class; Enlightenment / Marxist ideology captured the legitimacy narrative |
| **Theocracy** | Brittle to substrate falsification; vulnerable to schism | Late Caliphate; Counter-Reformation Spain; modern Iran | Ideology depends on metaphysical claims that material reality can contradict; schisms produce coordination failure when interpretive authority is contested |
| **Communist single-party** | Succession problem + information distortion + economic calculation failure | Soviet collapse (1991); Maoist Great Leap Forward | Single point of failure at top; honest reporting upward impossible (shoot-the-messenger); price signals absent (Mises/Hayek); each successor on average worse-selected than predecessor |
| **Fascist single-party** | Leader cult + war-economy dependence + successor void | Nazi Germany (1945); Mussolini's Italy (1943) | Mobilization requires expansion; ideology demands conflict with neighbors; no institutional continuity beyond the leader |
| **Liberal democracy (US/UK/EU)** | Elite capture → managerial-rentier class → fiscal indiscipline → asset-inflation transfer | Present (post-1971, accelerating post-2008) | Suppression stack (§1.2a) defers accountability events; counter-elite captured from inside the credentialing pipeline (§2.12A); coalition-bribery transfer crowds out substrate spending (§3.11) |
| **Party-state with technocratic competence (modern China)** | Succession risk + ideological brittleness + demographic collapse | In progress; test case under Xi-era consolidation | Concentration of power requires careful succession architecture; ideological commitment limits adaptive response to material reality; demographic substrate now exhausting (TFR ~1.0) |
| **Single autocrat (any era)** | Successor problem + leader-quality variance | Caesar→Augustus→Caligula→Claudius→Nero (Julio-Claudian arc) | First leader good; second great; third catastrophic; no filter beyond bloodline or whim; governance quality follows accidents of birth |
| **Tribal / Dunbar-scale** | Cannot scale beyond ~150 without importing one of the above failure modes | Every preliterate society throughout history | Above Dunbar, anonymous-cooperation infrastructure required; informal reputation tracking insufficient; any transition to a scaled form imports that form's failure mode |

**What the catalogue jointly forces.** Every historical system optimized for avoiding one failure mode and was killed by another. Athens prioritized broad participation and died from no continuity. Rome built the most sophisticated republican mixed constitution of antiquity and died from elite overproduction and military client-patronage. Feudalism solved local protection and died when protection nationalized but rent persisted. Absolutism solved feudal fragmentation and died of fiscal-military overreach. Liberal democracy solved arbitrary executive power and is dying of managerial-rentier capture. Communism solved capital concentration and died of information distortion and economic calculation. Fascism solved leftist class mobilization and died of war-economy dependence and successor void. **No historical system has avoided all of these failure modes simultaneously at civilizational scale.** Small-scale or specific-context survivors exist — Venice's commercial republic held ~1,000 years before Napoleon; Switzerland's federated direct democracy has held ~700; Iceland's Althing operated ~400 years before Norwegian conquest — but each operated at scales and under external conditions that did not test all the failure modes the present system must address, and none has been validated under the combination of population scale, technological substrate, and elite-capture pressure a modern successor would face. The design problem §4 addresses is therefore not "select among historical models" but "synthesize structural features no historical system has combined at scale."

**Mapping principles to failure modes.** Each §4.x principle below is the structural prophylactic against one or more catalogue entries:

- **§4.1 Cooperation (Axelrod Principles)** → against Roman civil-war defection dynamics, communist defection-from-honest-reporting, current accountability-vacuum-driven defection.
- **§4.2 Memory and Truth** → against Athenian no-continuity, communist information distortion, current epistemic fragmentation.
- **§4.3 Scale-Free Cooperation** → against the Dunbar limit; against every form's eventual scale-related failure.
- **§4.4 Sensemaking Infrastructure** → against communist information distortion, theocratic substrate-falsification brittleness, current epistemic fragmentation.
- **§4.5 Make Defection Costly** → against feudalism's protection-bargain hollowing, the accountability vacuum across imperial and modern forms.
- **§4.6 Accountability and Intent** → against imperial Roman succession coups, modern bureaucratic responsibility diffusion.
- **§4.7 Constraining and Aligning Elites** → against the Roman / feudal / absolutist / current elite-capture pattern.
- **§4.7A Collective Action Against Concentrated Private Power** → against feudal rent extraction, current asset-inflation rentier feudalization.
- **§4.8 Subsidiarity** → against absolutist over-centralization, communist top-down failure.
- **§4.9 Lifecycle Management** → against fascism's no-successor void, absolutism's heredity lottery, autocratic quality-variance.
- **§4.10 Voluntary Association and Exit Friction** → against communist exit denial; balanced against atomization-by-frictionless-exit.
- **§4.11 Continuous Adaptation** → against every system that ossified after initial success, including modern Western liberal democracy's current ossification.

The §4 specification is therefore not a wish list; it is a constraint set. Each principle traces to a known historical failure mode, and a successor system must satisfy every constraint simultaneously or it will fall to a failure mode that has already killed someone else's civilization.
...
**Trust is the output, not the input.** The standing objection to any design of this kind is that it presumes trust that does not exist and cannot be manufactured. The objection inverts the causation. High-trust societies are not populations that decided to be credulous; they are populations in which defection has been reliably detected and answered for long enough that trusting became the correct prior. Knack and Keefer (1997) measured the association between generalized trust and growth; Fukuyama's *Trust* (1995) supplies the mechanism—societies with a wide radius of trust form large-scale organizations spontaneously, while low-trust societies fall back on the family or on the state. What the Axelrod principles specify is not an appeal for more trust. It is the machinery that makes trust the rational default.

This also disposes of the demand for blind trust, which nobody is making. The Retaliatory principle *is* the answer to the skeptic: cooperation does not require thinking well of everyone, only that defectors be detected and answered. A system tolerates any number of would-be defectors provided the detection works.

**The failure mode is asymmetric detection, and it is the one we have.** The Retaliatory principle carries an unstated requirement: retaliation must be credible against defectors *at every level of the system*. A society that monitors and sanctions its ordinary members while its elite defects without consequence has not partially satisfied the principle—it has inverted it. The observable fact that defection at the top goes unanswered is itself a signal, and what it broadcasts is that the enforcement mechanism is decorative. Cooperators who read that signal correctly downgrade to defection wherever they can. This is the precise mechanism by which *rules for thee but not for me* destroys a high-trust equilibrium: not through the direct cost of elite extraction, which is usually modest as a share of output, but through the destruction of the common knowledge that made everyone else's cooperation rational.
...
**Exit must be equally available and equally priced:** An exit only the resourced can exercise is not an exit right; it is an elite privilege that destroys voice for everyone else. But the correction is not to make exit cheap for all — Document 3 §5.4 argues that cheap exit hollows a polity fastest, with Venezuela and Ecuador as the cases. The requirement is that one rule apply to everyone: free for the person, priced against accumulated claim, with the price scaling to what was taken from the collective rather than to the departer's wish to go. That is what symmetry means here. A rule that is nominally universal but priced so that only the wealthy can meet it is the same asymmetry under another name.
...
**Latin America is the natural experiment, and it is unusually clean because the text was held constant.** Nineteenth-century republics across the continent adopted constitutions closely modelled on the American one — Argentina's 1853 constitution was drafted from Alberdi's explicit study of it, and Mexico, Brazil and Venezuela produced close relatives. The enumerations were the same. Separation of powers, federalism, enumerated rights, judicial review: all present, all on paper. What followed was caudillismo, and whoever held the presidency held, in practice, the powers of a king. Venezuela has since run through roughly two dozen constitutions; the Dominican Republic has had more than thirty. The documents were not the variable.

What was missing was everything underneath: an independent bar, a church that was a rival rather than a client, chartered municipalities with their own revenue, commercial houses that could survive a government's displeasure, a press with an owner who did not need a licence. The North American colonies arrived with those bodies already built and the constitution ratified an arrangement that largely existed. The southern republics wrote the same words over a society with one real centre of power, and the words did what words do on their own, which is nothing. This is the strongest available evidence that enumerations are downstream: hold the text fixed, vary the enforcing bodies, and the outcome varies completely.
...
**2. Exit rights as floor, not as the check:**
- Voluntary association means individuals must be able to leave communities that violate their rights. That floor is not negotiable: the person may always go
- But exit is not the disciplining mechanism it is usually taken for. §3.1b and Document 3 §5.4 establish close to the opposite — where exit is cheap it substitutes for voice, and the members most capable of repairing an institution are the first to leave
- So exit is priced against accumulated claim rather than free, and its function is *audit* rather than discipline: a community that cannot lose members never has to justify what it asks of them
- The check itself is plural power centres with independent means, per §3.1b. Multiple overlapping communities matter because they are rival centres holding each other to terms, not merely because they are alternative destinations
...
**Connection to other principles:** Rights subsidiarity is subsidiarity applied to power allocation. Note that §3.1b treats "default to individual, justify collective" as a malformed framing rather than a settled default: the operative metric there is how many independent centres hold both capacity and standing interest, under the three constraints (plural, contestable, capped at the boundary). Exit contributes to that by keeping the terms honest, not by supplying the competitive pressure on its own. Grants of collective authority should sunset and require renewal, not persist indefinitely. Concentrated power enables rights violations; distributed authority reduces risk. Rights frameworks can evolve as technology and threats change, without requiring constitutional crises.
...
The three axes originate with Richard Shweder's cross-cultural work on moral reasoning — the "big three" ethics of **autonomy, community and divinity** — which Jonathan Haidt takes up and develops in *The Righteous Mind*. This document uses them as Individual (autonomy, agency, self-expression), Community (belonging, cooperation, shared purpose), and Transcendence (meaning beyond self, connection to something larger). The substitution of *transcendence* for Shweder's *divinity* is deliberate and worth flagging: the claim here concerns the sensor and what it is for, not religion specifically, and the religious forms are one way of feeding it rather than the thing itself. Just as healthy organisms require coordination across scales—individuals forming teams, teams forming institutions, institutions forming civilizations—**healthy civilizations require balance across these three axes**.
...
### 4.23 Differentiated Cohort Architecture

Universal undifferentiated franchise treats decisions of different character and stake as if they required the same competence and the same aggregation channel. The resulting failure modes are documented in Doc 1 §2.9 — small-coalition capture from above, dependent-population coalition formation from below, leaders selected for intra-party survival rather than substrate governance. Differentiating the franchise — different cohorts with different powers — is permitted and historically common (Venice, Athens, Rome, Switzerland, Singapore). The hard problem is that the differentiation itself becomes a capture surface: a smaller, more powerful cohort has every incentive to rewrite the qualifying rules to entrench its position. Without structural anchoring, every differentiated-franchise system collapses into oligarchy.

**Requirement:** Differentiation is structurally permitted but anchored such that meta-capture is impossible.

- **Multiple cohorts with different powers may exist.** Different decisions may be assigned to different cohorts based on stake, competence, function, or any other principle. The specific assignment — what cohorts exist, what they decide, on what timescale — is a Document 3 mechanism question.

- **The largest cohort is unbounded in scope.** There exists a broadest cohort whose membership cannot be reduced, filtered, or qualified-out by any narrower cohort. Its qualifying basis is membership in the polity itself. No smaller cohort has authority to shrink it.

- **The largest cohort controls the rules governing selection into all other cohorts.** Who qualifies for which cohort, by what criteria, under what review, is set by the broadest cohort and only by it. Smaller cohorts cannot self-define, expand their own scope, or alter their qualifying criteria. They operate within rules they did not write and cannot rewrite.

- **Narrower cohorts must be curated, and the curation mechanism is an open problem.** Selection into narrower cohorts must occur on the basis of properties the cohort actually depends on — institutional discernment, curatorial instinct, demonstrated stake, judgment, civic competence, technical ability — rather than on demographic membership. Demographic correlations with these properties exist (Doc 1 §1.4, §2.9) and are diagnostic information; the qualifying *rule itself* cannot be the demographic correlate. Beyond this, the spec must be honest about its limits: Goodhart's law applies to every specific selection mechanism — tests get gamed, credentials get captured, peer review degrades, primaries get captured. Civil service exams, SAT/IQ batteries, professional licensure, and electoral selection have all degenerated under sustained gaming pressure. The discovery of curation mechanisms that resist gaming — and the institutional design that allows mechanism revision when gaming inevitably emerges — is a central open problem taken up in Document 3. The spec asserts only that curation must occur, cannot be demographic, and must remain revisable; it does not pretend to know the right mechanism.

- **The broadest cohort's authority must be directly exercisable, not delegated through narrower cohorts.** The mechanism by which the broadest cohort sets and alters the meta-rule cannot itself be controlled, gated, or filtered by any narrower cohort — otherwise the meta-rule is recaptured at the meta-mechanism layer. The canonical failure mode is US congressional term limits: ~80% sustained public support, no mechanism for direct exercise, every constitutional amendment path runs through Congress itself (Article V) or through state legislatures that *Thornton* (1995) confirmed cannot add qualifications. The smaller cohort retains structural control over the rules of its own qualification by controlling every path to changing those rules. Switzerland's popular-initiative-plus-referendum is the working operationalization: 100,000 citizens plus a national majority can amend the federal constitution without parliamentary consent.

- **Negative authority operates at lower thresholds than positive authority.** Veto, removal, revocation, and blocking by the broadest cohort operate at lower thresholds than additions or positive structural changes. Removing or blocking is more easily reversed than building, and a thin majority adding poorly-considered structural changes can do greater damage than a minority blocking well-considered ones. Illustrative thresholds — alarm-bell veto at ~40%, ordinary positive legislation at majority, structural changes to the cohort architecture at 2/3 to 4/5 — are empirically tunable parameters subject to refinement and testing across implementations; the structural property is the *direction of asymmetry*, not the specific numbers. The asymmetry encodes the Hippocratic principle: easier to stop than to start, easier to remove than to install, status quo privileged against thin-majority alteration.

The third through sixth points are the load-bearing ones. It prevents the recurring failure mode of differentiated systems — the more powerful cohort rewriting the meta-rule to entrench itself — because the meta-rule is structurally not within its authority. The broadest cohort's authority over the meta-rule is constitutive, not consultative. Ordinary policy authority may be assigned away to specialized cohorts; the structure of the differentiation itself cannot be.

**The trade-off is real.** Anchoring constitutive authority in the broadest cohort means low-information majorities can in principle rewrite the qualifying rules in destructive ways. The alternative — letting any narrower cohort write the meta-rule — guarantees capture on a longer time horizon. The functioning historical precedents share the same anchor: ultimate authority over the structure rests with the broadest possible cohort; ordinary authority is differentiated below it.

Specific mechanisms — what the cohorts are, what powers each has, how qualification is administered, what thresholds the broadest cohort needs to alter the structure, and what protections exist against alteration that would itself constitute meta-capture — are catalogued in Document 3.
