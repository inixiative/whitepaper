...
**This describes human cultural behavior better than market economics does.** Rational choice theory predicts that agents maximize fitness given information and incentives. But humans observably and systematically choose conspicuously fitness-reducing options — and not as errors or information failures, but as *preferences*:
...
4. **The Weinstein gap.** The deepest problem remains Weinstein's: we do not have a satisfying explanation for why females select for conspicuously fitness-reducing traits in the first place. The models describe stable equilibria but not why organisms enter them. Until evolutionary biology resolves this — if it can — the cultural application inherits the same explanatory gap. We are applying a pattern that demonstrably exists to a domain where it demonstrably fits, while acknowledging that the pattern itself is not fully understood.
