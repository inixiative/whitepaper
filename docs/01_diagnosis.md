...
Neuroscientist Iain McGilchrist's work on hemispheric brain function reveals why we systematically fail to see what systems actually do—and why cover stories work so effectively. The underlying finding is Michael Gazzaniga's, from the split-brain experiments: the left hemisphere runs an *interpreter* that generates coherent, plausible narratives even when it has no access to the actual cause. Shown a pattern it cannot explain, it confabulates — produces a story that sounds right, delivered with full confidence and no sense of having invented anything. McGilchrist's own contribution is the larger claim built on top of that: which mode of attention a culture comes to privilege, and what it stops being able to see as a result.
...
**The 10% tipping point:** Network science confirms this isn't theoretical. Research at Rensselaer Polytechnic Institute (2011) modelled this and found a threshold near **10-11% committed believers**, above which the belief propagates to the majority and below which it effectively never does — the paper's phrase for the sub-threshold case being that propagation would take "the amount of time comparable to the age of the universe." Two cautions belong on the number, because it is usually quoted without them. It is an agent-based simulation, not an observed law about real populations, and the threshold moves with the network topology assumed. What survives the caveats is the shape rather than the constant: committed minorities propagate non-linearly, and there is some threshold below which they do not propagate at all. The selectorate doesn't need to be large; it needs to be **committed and coordinated**. This is why focused minorities (activists, lobbies, ideological movements) punch far above their numerical weight, while diffuse majorities with mild preferences never organize. The 80% majority doesn't coalesce because their preferences lack mimetic intensity—they feel support but don't see others demonstrating passionate commitment, so they assume their position is weak.
