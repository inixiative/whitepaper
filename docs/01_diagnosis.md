...
**The Cooperation-Radius Destruction Mechanism**
...
This is not a conspiracy theory. It's a straightforward case of optimizing for the wrong objective function. Elites optimized for *manageability*. Manageability is the opposite of the capacity for self-governance. A manageable population is one whose cooperation radius has been contracted to "do what you're told" — exactly the cooperation-radius contraction that Section 1.4 identifies as the core civilizational failure mode. The elites got what they optimized for, and what they optimized for is killing the system.
...
**Spatial Complexity and the Density Signal**

But Universe 25 was not Calhoun's only experiment. His variations with different spatial architectures — more side rooms, multiple levels, greater physical complexity — delayed or attenuated the behavioral sink. The mice still hit competitive saturation eventually, but spatial complexity bought time and reduced severity. More defensible private territory per individual meant the competitive intensity calibration didn't go into overdrive as fast. The architecture of the physical environment modulated the density signal.

The human parallel is visible in the data. The biological density sensor doesn't read property deeds or understand rent markets — it reads the same cues Calhoun's mice read: how many conspecifics are you encountering in your territory, how much space can you defensibly control, how often are you in involuntary proximity to competitors. Japan (some of the smallest average dwelling sizes in the developed world, 1.2 fertility), South Korea (extreme density in Seoul plus tiny apartments, 0.72 fertility), and Hong Kong (the smallest average dwelling sizes of any major city, 0.8 fertility) show the predicted pattern: maximum-density signals correlating with maximum fertility suppression, regardless of material wealth.

The critical variable is not density per se — you can stack people vertically — but density *without spatial complexity*: without enough defensible private territory per individual that the biological sensor reads "I have space" rather than "I'm surrounded by competitors." The architecture of the built environment is an input to the density sensor, and the density sensor drives competitive intensity calibration. Calhoun demonstrated this experimentally with mice. The cross-national fertility data suggests the same mechanism operates in humans.
