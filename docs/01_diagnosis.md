...
### 2.12A The Illegible Channel: Relationship-Contingent Rule Application
...
- **Mode A — bending rules for material gain.** Detectable in principle. Financial audit, conflict-of-interest disclosure, and procurement review exist precisely for this. Experimental evidence on deception for monetary gain shows a male skew.
...
**Relationship-contingency is worse than leniency.** A uniformly lenient adjudicator is *legible*: predictable, modelable, correctable, routable-around. Relationship-contingent leniency is illegible by construction—the identical rule yields different outcomes as a function of an input the institution never records. This is metis destruction (§1.12) running in reverse: not the state failing to see local knowledge, but the institution failing to see itself.

**The evidence on sex differences, stated at its actual strength.** Two independent methodologies converge. In third-party punishment experiments, men anchor punishment to the in-group and show out-group derogation, while women anchor to the out-group and show **in-group favoritism**—favoritism in punishment allocation being unequal rule-application by relationship, measured directly. In judicial sentencing, the literature is mixed overall but carries a recurring lenience pattern and one result matching the lab: female judges sentence female defendants more leniently than male judges do. Dreher et al. supply the substrate: testosterone increases *both* punishment of unfair offers and reward of generous ones—norm enforcement as such, not aggression.

Caveats belong in the same breath. The judicial literature is genuinely mixed. Effects are domain-dependent—women are *more* punitive regarding sexual offenses and crimes against children. The favoritism result wants replication. Effect sizes are not established, and **the magnitude of any institutional contribution is unidentified**. What is defensible is the direction and the convergence, not a coefficient.

**Why the two channels have different stopping rules.** The asymmetry is not arbitrary; it follows from payoff structure. For a man, eliminating a same-sex rival costs a fighter. The rival retains coalition value *as a rival*—tough men are wanted precisely because they are tougher than the other men, and a larger home fighting force raises the odds in intergroup conflict. Male intrasexual competition therefore terminates at status adjustment: contest, resolve rank, reconcile, retain. Social exclusion is a last resort. For a woman, the same-sex rival carries no offsetting value: fewer unrelated women means more food, territory, allies, mates, and high-status positions for her and her children, and because marriage pairs one woman to one man, a single additional woman can be the difference between married-with-resources and poor single mother. Benenson states the consequence directly: *there is no obvious downside to eliminating a female from the community—less really is more.* Exclusion is first-line rather than last, and it has **no natural terminus**, because elimination is the payoff rather than a byproduct of a status contest.

This is the mechanism beneath the register difference. Open hierarchical competition is self-limiting by construction. Relational exclusion is not.

**Duty cycle, not rate.** The absence of a terminus has a second consequence. A contest that resolves has an *off* state: men contest, settle rank, reconcile, and stand down. A mode with no settlement ritual has nothing to switch off, so competition runs continuously rather than episodically—ambient rather than event-driven. And because covert competition requires concealment to function at all (one cannot be visibly competing), the mode carries a *standing* requirement for self-concealment where overt competition carries only an intermittent one. Male confabulation is largely situational—post-hoc justification of an outcome already achieved, of which §2.10's merit narrative is the type case. The relational mode requires it continuously.

The distinction is more consequential than the per-occasion question, and it also dissolves it. Discrete-decision paradigms—the private die-roll and its relatives—show a male skew, but those instruments measure a *single act* under explicit incentive, which is the overt channel by construction. They are shape-mismatched to a continuous baseline state, so they establish nothing about per-occasion distortion within the covert channel in either direction. The nulls are not evidence of absence; the instrument is the wrong shape. What can be said is structural: one mode runs intermittently and the other runs always.

**Intensity and duration point the same direction.** There is no offsetting term here. Benenson's "worriers" thesis is that women are *more* vigilant to same-sex threat, more affected by social conflict, and more continuously monitoring it—the baseline is not a low-intensity one traded against male peaks. Higher sensitivity and no off state compound rather than cancel, which yields a straightforward directional expectation: greater time-integrated confabulation load. The magnitude of that gap is unmeasured; its direction is derived from premises, not assumed.

The institutional consequence compounds further, because *distribution* matters independently of quantity. An episodic distortion presents as an event, and events can be investigated. A continuous distortion becomes the baseline against which everything else is measured, and produces no incident to audit. Even setting aside the higher expected load, the continuous form is worse for legibility—which is why the mechanisms in §6.8A operate on aggregate variance rather than case review: there is no case.

**The signature this predicts, and a correction it forces.** The male-coded institutional failures usually offered as counterexamples—clerical abuse concealment, the Soviet nomenklatura, pre-2008 trading desks—share a feature that is easy to miss: all three *concealed* their insiders. They protected the abusers, the apparatchiks, the traders. That is the coalition-value stopping rule operating exactly as predicted: you hide your own because you still need them. The exclusion dynamic does the opposite—it destroys insiders. Concealment and elimination are opposite signatures on the same axis, which is evidence that the channel difference is doing explanatory work that "illegible self-advantaging" alone does not capture.

**The stopping rule is purchased by external threat, not by sex.** The male terminus exists because an enemy exists. Remove intergroup competition and the rival's coalition value goes to zero for everyone, at which point exclusion becomes rational irrespective of composition. This is the same mechanism §1.13 identifies in governance systems that lose competitive pressure, and the same one behind prosperity eliminating defensive capability. An institution that stops perceiving an outside adversary will converge on the exclusion repertoire whoever staffs it—which predicts that restoring a legible external standard of success is itself a corrective, and that institutions insulated from outside competition should degrade first.

**Vehicle or arena: the perceptual layer underneath all of it.** Every actor inside an institution holds two magnitudes in some ratio—the value of the institution itself, and the value of winning the competition occurring within it. The failure diagnosed here is not that anyone consciously discounts the first. It is that for a substantial population the first is not *perceived* at the same intensity as the second, and no trade-off can be made against a quantity that does not register.

Male intrasexual competition is coalitional: one wins by fielding a stronger group than the opposing group. Competitive success therefore runs *through* the institution, whose health is directly instrumented in the individual's own position. This requires no altruism—degrading the institution weakens the instrument one is competing with. The institution is a **vehicle**, and a vehicle is maintained because it is being ridden.

Female intrasexual competition is positional within the group: success is measured against the proximate same-sex peer rather than against a rival coalition. The institution's health does not sit in the causal path of competitive success. It is an **arena**—the place where the contest occurs. Arenas are not maintained. This is not a decision to neglect; maintenance is simply not a component of the activity.
...
**Two independent confirmations that this was not constructed to produce.** First, it re-derives the external-threat result. Vehicle-perception depends on the existence of a rival coalition; remove the outgroup and the institution ceases to be a vehicle for anyone, becoming an arena for men as well. The perception is not male—it is *purchased by intergroup competition*, which is the conclusion the stopping-rule argument reached by a different route.

Second, it survives the obvious objection. Men damage institutions constantly through looting, careerism, and empire-building. But observe the *form*: they extract from a functioning institution rather than destroying it. One does not burn what one is riding. Clerical concealment, the nomenklatura, and pre-2008 trading desks all hid wrongdoing in order to keep the vehicle operating—corruptly, but protectively. Male-coded institutional failure is **parasitism**; arena-degradation is a distinct signature. Both are destructive and the two are distinguishable, which is what makes the claim falsifiable rather than a general indictment.

Stated as a mechanism rather than an accusation: no one need intend the institution harm. A competitor who does not perceive it as a vehicle has no reason to spare it, and no awareness that there was anything to spare.

**The coverage gap.** Institutions were instrumented to detect male-typical failure modes: overt power seizure, visible corruption, open factional conflict. Their sensors, escalation paths, and audit mechanisms all assume the failure will be *visible* and *strategic*. The relational channel trips none of them. This is not a claim that anyone's perception is deficient—it is Doc 2's umwelt argument applied to institutions: *"This isn't stupidity or malice—it's the structural fact that perception is bounded while reality is not."* Institutions inherit the perceptual bounds of their builders. Change the composition without re-instrumenting and failure modes accumulate that no sensor registers.

**Why strategic concealment is easier to fix than self-blind concealment.** Patronage, nepotism, and insider dealing are concealed from the institution but *known to their author*. They can be interrogated; the actor can be induced to disclose. Self-blind rule-variance cannot—sincere denial survives interrogation that strategic denial does not. Every male-coded cover story of the modern era (divine right, national security, efficient markets) was eventually examined and defeated. The question is whether the current one can be.

**The adjudication layer fails the same way.** The coverage gap has a second half. Detection fails because no sensor registers relationship-contingent variance; *adjudication* fails for a separate reason, derivable from two further premises about non-kin cooperation.

The first is the default classification of a colleague. Male-typical coalition formation treats a non-kin peer as a teammate by default—useful against the outside, therefore retained. The female-typical pattern treats a non-kin same-sex peer as a competitor by default, with cooperation required to earn its keep. Institutions are constructed on the first assumption: shared credit, team-based evaluation, open information flow, and mentorship all presuppose that colleagues are default allies. Under the competitor default those same structures are not used but *exploited*—information becomes costly to share, a peer's failure becomes one's own gain, and their removal carries no offsetting loss.

The second is the threshold for rupture. Male-typical relationship termination requires something like betrayal: a violation of an agreed rule. The female-typical threshold is lower and differently constituted—an action that produced a bad feeling suffices, with no rule violation required, and Benenson's documented finding is that girls' friendships dissolve after conflict where boys' reconcile. Translate that criterion into an institutional setting and **the trigger for sanction detaches from rule violation entirely**.
...
This subsection is a derivation rather than a body of evidence: it states what follows *if* the premises hold. The second premise carries most of the weight, and its documented form is narrower than the general claim—lower rupture threshold and absent reconciliation, rather than a measured claim that no wrongdoing is required.
...
This is the fence failure in its exact form: **the stated reason was never the operative reason, only the available one.** Refuting the stated reason licenses nothing about the fence. And the knowledge in question is metis (§1.11)—embedded in practice, accumulated through differential survival rather than through understanding, and therefore structurally unable to defend itself when a high-modernist reform demands articulate justification. The mechanism by which such knowledge is destroyed is the subject of §1.11; this is that mechanism applied to one specific fence.
...
**Note what this does to the selection problem.** A regime that makes the *ratio* the object of enforcement rather than the *trait* has the same structure as pretextual proxy selection—choosing screens for their demographic output rather than their predictive validity—operating in the opposite direction. Both sever the link between selection criteria and the thing the criteria exist to measure. An institution forbidden to use a validated predictor because the predictor produces a disparity is an institution that has been legally deprived of the ability to select for competence. This is the substrate mechanism of the preceding paragraph, supplied not by preference but by statute. §4.22 states the corresponding requirement.
...
**The renewal is not stored; it decays with the perception that sustains it.** Vehicle-perception is not deposited in institutional structure—it is held by people who believe the threat, and it degrades on the schedule of the generation holding it. This is the Forgetting Loop (§2.15) operating on the one variable that reverses institutional decay, and it places the post-1945 Western arrangement approximately where §2.13 locates it.
...
This is the Forgetting Loop (§2.15) operating on the variable that determines whether a crisis resolves or ruptures, and it is the same mechanism as vehicle-perception: believing an existential stake requires having seen the downside. It also gives the practical form of the earlier point—what tips an elite toward accommodation is not proximity to conflict but proximity to *losing*. One that won without changing learned that not changing works.
