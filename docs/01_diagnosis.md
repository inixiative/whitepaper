...
The distinction matters more than any claim about rates. It is not that one sex generates more distortion per occasion—the evidence does not support that, and the cleanly-measured cousins of confabulation skew male. It is that one mode runs intermittently and the other runs always. This also explains why the experimental literature registers so little: paradigms like the private die-roll measure a *discrete decision*, and an instrument built for discrete acts cannot detect a continuous baseline state. The nulls are not evidence of absence; the instrument is the wrong shape.
