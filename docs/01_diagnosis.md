...
Note the institutional consequence is independent of whether the aggregate quantity is in fact larger—continuous-low and episodic-high may well integrate to the same total. What differs is *distribution*. An episodic distortion presents as an event, and events can be investigated. A continuous distortion becomes the baseline against which everything else is measured, and produces no incident to audit. At equal magnitude the continuous form is strictly worse for legibility, which is why the mechanisms in §6.8A operate on aggregate variance rather than case review: there is no case.
