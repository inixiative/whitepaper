...
**The retention disposition is male-coded; external threat governs its salience, not its existence.** The terminus is an evolved disposition calibrated to ancestral intergroup conflict, and its selection criterion is *contribution capacity* rather than agreement: keep anyone who could contribute, and the formidable especially, precisely because they are more formidable than the others. Because the criterion is orthogonal to whether a person agrees with you, it extends directly to those holding opposing views and not merely to status rivals. This is the psychological substrate of tolerated opposition—loyal opposition, the standing dissenter, the rival kept inside the tent.

Dispositions do not recompute when conditions change. This is the same evolutionary-mismatch premise operating elsewhere in this work, where physical formidability continues to predict political attitudes despite being irrelevant to modern economic competition, because the psychology is calibrated to ancestral rather than current environments. Removing the outgroup therefore does not abolish the retention disposition; it removes the urgency that makes acting on it obviously worthwhile. Degradation under prolonged absence of external threat is partial and lagged, not complete—which is consistent with §1.13 on governance systems losing competitive pressure, and with the convergent finding that vehicle-perception weakens under the same condition.

**Scope condition.** The retention rule is conditional on the coalition being defined by a *task*. Where a coalition is instead defined by a *creed*, conformity becomes the contribution, and the same rule resolves toward expulsion—which is why ideologically-constituted male institutions purge as readily as any other. This is a limit on when the disposition protects against homogenization, not a counterexample to it.

**Why the resulting homogeneity is dangerous, not merely uniform.** Tolerated opposition is what preserves internal variance, and variance is what keeps failure modes *uncorrelated*. A population stripped of variance fails simultaneously rather than sequentially, which is the actual content of brittleness—the Gros Michel banana and the Irish potato are the standard illustrations, and the mechanism transfers directly. In cybernetic terms this is Ashby's law of requisite variety: a controller must possess at least as much variety as the system it regulates, so an institution that has purged internal variance can no longer model an environment more varied than itself. That is signal-fidelity failure arriving from the population side rather than the sensor side.

The prediction worth testing is that this is invisible until it is expensive. Monocultures are genuinely *efficient* under stable conditions—reduced internal friction, faster consensus, lower coordination cost—which is both why they arise and why nobody corrects them. Brittleness manifests only under perturbation. Exclusion-dominant institutions should therefore **outperform on routine throughput and fail catastrophically under shock**, with the failure arriving without warning precisely because stable-period performance was real.
